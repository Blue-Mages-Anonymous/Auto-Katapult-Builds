...
KATAPULT_BUILD_DIR="$(pwd)/katapult"
...
get_latest_katapult_version() {
    cd "$KATAPULT_BUILD_DIR"
    git fetch --tags
...
build_firmware() {
    ...
    if [[ "$board_config" == *"pico"* ]]; then
        # For RP2040 boards
        cp out/katapult.uf2 "$FIRMWARE_OUTPUT_DIR/${board_config}.uf2"
    else
        # For other boards
        cp out/katapult.bin "$FIRMWARE_OUTPUT_DIR/${board_config}.bin"
    fi
}
...
main() {
    ...
    echo "Katapult build directory will be: $KATAPULT_BUILD_DIR"
    
    # Clone or update Katapult repository
    if [ ! -d "$KATAPULT_BUILD_DIR" ]; then
        echo "Cloning Katapult to $KATAPULT_BUILD_DIR"
        git clone "$KATAPULT_REPO" "$KATAPULT_BUILD_DIR"
    else
        echo "Updating existing Katapult repository"
        cd "$KATAPULT_BUILD_DIR"
        git fetch origin
    ...
    # Get latest version
    latest_version=$(get_latest_katapult_version)
    echo "Latest Katapult version: $latest_version"
